...
<template>
  <div class="container">
    ...
    <div class="row">
      ...
      <!-- 두번째 -->
      <!-- <button class="btn btn-success" @click="isUpdated ? empUpdate() : empInsert()">저장</button> 하나의버튼에 두개의핸들러를 넣는방식-->
    </div>
  </div>
</template>
<script>
...
export default {
    data() {
        return {
            isUpdated : false, // 등록이 기본베이스, false면 등록모드
            searchNo: '',
            empInfo: {
                emp_no : '',
                first_name : '',
    ...
    created() {
        ...
        if(this.searchNo > 0){
            this.getEmpInfo();//
            this.isUpdated = true; //급여와 소속부서 선택을 막음
        }else{
